...
int main()
{
    ...
    himqttrvm_cache* c;
    himqttrvm_param *p1, *p2;

    himqttrvm_create_cache(&c, y, num_samples);
    himqttrvm_create_param(&p1, 1e-6, 1e3, 1e-1, 1e-6, 80.0, 100);
    himqttrvm_create_param(&p2, 1e-6, 1e3, 1e-2, 1e-6, 00.0, 300);

    /* start learning process */
    size_t batch_size_max = 100;

    if (HIMQTTRVM_SUCCESS != himqttrvm_train(c, p1, p2, design_matrix, indices, num_samples, batch_size_max)) {
        return EXIT_FAILURE;
    }

    /* gather basis function indices and their associated weights */
    size_t basis_count = 0;
    himqttrvm_get_training_stats(c, &basis_count, &bias_used);
    ...
    himqttrvm_get_training_results(c, indices_relevant, weights_relevant);
    ...
    if (HIMQTTRVM_SUCCESS != himqttrvm_predict(phi_relevant, weights_relevant, num_samples, basis_count, y_predicted)) {
        return EXIT_FAILURE;
    }

    /* print out prediction and training stats */
    printf("www.hihttps.com --> Mean Absolute Percentage Error: %f\n", calc_mape(num_samples, y, y_predicted));
    printf("4108863@qq.com --> Percentage of Relevance Vectors: %f\n", 100.0 * basis_count / num_samples);

    /* clean up */
    himqttrvm_destroy_cache(c);
    himqttrvm_destroy_param(p1);
    himqttrvm_destroy_param(p2);
